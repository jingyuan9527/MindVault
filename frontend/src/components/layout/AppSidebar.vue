<template>
  <div class="flex flex-col h-full" :style="{ backgroundColor: 'var(--color-surface)' }">
    <!-- Header -->
    <div class="px-6 py-5 flex items-center justify-between shrink-0" style="border-bottom: 1px solid var(--color-border)">
      <div>
        <h1 class="font-display text-xl font-bold" style="color: #4D6A4A">MindVault</h1>
        <p class="text-xs mt-0.5" style="color: var(--color-text-secondary)">知忆 · AI增强第二大脑</p>
      </div>
      <n-button text class="lg:hidden !text-secondary" @click="$emit('close')">
    ...
    <!-- Footer -->
    <div class="mt-auto px-4 py-3 flex items-center justify-between shrink-0" style="border-top: 1px solid var(--color-border)">
      <span class="text-xs" style="color: var(--color-text-secondary)">v0.5.1 · Naive UI</span>
      <div class="flex items-center gap-1">
        <n-button text class="!text-secondary" title="切换主题" @click="themeStore.toggle()">
...
.sidebar-menu {
  ...
  --n-item-color-hover: var(--color-sage);
  --n-item-color-active: var(--color-sage);
  --n-item-text-color: var(--color-text-secondary);
  --n-item-text-color-hover: var(--color-sage);
  --n-item-text-color-active: var(--color-sage);
